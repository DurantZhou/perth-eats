...
const INIT_RESTAURANTS = [
  {id:1,name:"Wahu",cuisine:"Japanese",suburb:"Northbridge",price:2,meal:["lunch","dinner"],vibe:"Ramen & izakaya vibes",rating:4.6},
  {id:2,name:"Young George",cuisine:"Modern Australian",suburb:"Victoria Park",price:3,meal:["dinner","brunch"],vibe:"Neighbourhood gem",rating:4.7},
  {id:3,name:"Petition Kitchen",cuisine:"Modern Australian",suburb:"Perth CBD",price:3,meal:["lunch","dinner","brunch"],vibe:"Sophisticated & chic",rating:4.5},
  {id:4,name:"El Público",cuisine:"Mexican",suburb:"Northbridge",price:2,meal:["dinner"],vibe:"Loud, fun, taco heaven",rating:4.6},
  {id:5,name:"Sauma",cuisine:"Indian",suburb:"Northbridge",price:2,meal:["dinner"],vibe:"Modern Indian small plates",rating:4.5},
  {id:6,name:"Mary Street Bakery",cuisine:"Cafe / Brunch",suburb:"Highgate",price:1,meal:["brunch","lunch"],vibe:"Best brunch in Perth",rating:4.7},
  {id:7,name:"Nao Japanese",cuisine:"Japanese",suburb:"Subiaco",price:3,meal:["lunch","dinner"],vibe:"Omakase & sushi",rating:4.8},
  {id:8,name:"Lalla Rookh",cuisine:"Italian",suburb:"Perth CBD",price:3,meal:["lunch","dinner"],vibe:"Wine bar & pasta",rating:4.5},
  {id:9,name:"Balthazar",cuisine:"Modern Australian",suburb:"Perth CBD",price:4,meal:["dinner"],vibe:"Fine dining icon",rating:4.6},
  {id:10,name:"Hawker's Quarter",cuisine:"Asian",suburb:"Subiaco",price:1,meal:["lunch","dinner"],vibe:"Asian street food court",rating:4.3},
  {id:11,name:"Long Chim",cuisine:"Thai",suburb:"Perth CBD",price:2,meal:["dinner"],vibe:"David Thompson's Thai kitchen",rating:4.5},
  {id:12,name:"Chu Bakery",cuisine:"Cafe / Brunch",suburb:"Northbridge",price:1,meal:["brunch","lunch"],vibe:"Asian-fusion pastries",rating:4.6},
  {id:13,name:"Nobu",cuisine:"Japanese",suburb:"Elizabeth Quay",price:4,meal:["dinner"],vibe:"World-famous Japanese fusion",rating:4.7},
  {id:14,name:"Modo Mio",cuisine:"Italian",suburb:"Northbridge",price:2,meal:["dinner"],vibe:"Family trattoria feel",rating:4.4},
  {id:15,name:"Ethos Deli",cuisine:"Cafe / Brunch",suburb:"Perth CBD",price:2,meal:["brunch","lunch"],vibe:"Deli & weekend brunch spot",rating:4.5},
  {id:16,name:"Must Winebar",cuisine:"French",suburb:"Mount Hawthorn",price:3,meal:["dinner"],vibe:"Rustic French classics",rating:4.6},
  {id:17,name:"Mechita",cuisine:"Latin American",suburb:"Northbridge",price:2,meal:["dinner","brunch"],vibe:"Vibrant South American",rating:4.4},
  {id:18,name:"Brika",cuisine:"Greek",suburb:"Highgate",price:2,meal:["dinner","brunch"],vibe:"Modern mezze & small plates",rating:4.5},
  {id:19,name:"Pho Deluxe",cuisine:"Vietnamese",suburb:"Northbridge",price:1,meal:["lunch","dinner"],vibe:"Authentic pho & banh mi",rating:4.4},
  {id:20,name:"Print Hall",cuisine:"Modern Australian",suburb:"Perth CBD",price:4,meal:["lunch","dinner"],vibe:"Grand heritage building",rating:4.5},
  {id:21,name:"Bivouac",cuisine:"Cafe / Brunch",suburb:"Mount Lawley",price:2,meal:["brunch","lunch"],vibe:"Artsy neighbourhood cafe",rating:4.4},
  {id:22,name:"Garum",cuisine:"Italian",suburb:"Perth CBD",price:3,meal:["lunch","dinner"],vibe:"Roman-style trattoria",rating:4.6},
  {id:23,name:"Tiger Lily",cuisine:"Asian",suburb:"Subiaco",price:2,meal:["dinner"],vibe:"Pan-Asian small plates",rating:4.3},
  {id:24,name:"Phat Boy",cuisine:"Asian",suburb:"Northbridge",price:1,meal:["lunch","dinner"],vibe:"Bold Asian street food",rating:4.2},
  {id:25,name:"Shuk",cuisine:"Middle Eastern",suburb:"Mount Hawthorn",price:2,meal:["brunch","lunch","dinner"],vibe:"Israeli-inspired all-day dining",rating:4.6},
  {id:26,name:"Saisaki",cuisine:"Japanese",suburb:"Perth CBD",price:2,meal:["dinner"],vibe:"All-you-can-eat sushi & teppan",rating:4.1},
  {id:27,name:"The Flour Factory",cuisine:"Modern Australian",suburb:"Perth CBD",price:2,meal:["dinner","brunch"],vibe:"Cocktail bar & share plates",rating:4.4},
  {id:28,name:"Co-op Dining",cuisine:"Modern Australian",suburb:"Mount Lawley",price:3,meal:["dinner"],vibe:"Intimate chef's table feel",rating:4.7},
  {id:29,name:"Duong Saigon",cuisine:"Vietnamese",suburb:"Northbridge",price:1,meal:["lunch","dinner"],vibe:"Casual banh mi & noodles",rating:4.3},
  {id:30,name:"Bread in Common",cuisine:"Modern Australian",suburb:"Fremantle",price:2,meal:["lunch","dinner","brunch"],vibe:"Fremantle institution",rating:4.5},
];
...
function Login({onLogin}) {
  ...
  const getUsers = () => { try{return JSON.parse(localStorage.getItem("pe_users")||"{}")}catch{return{}} };
  const submit = () => {
    ...
    if(mode==="signup"){
      if(users[username.toLowerCase()]){setErr("Username already taken.");return;}
      users[username.toLowerCase()]={username:username.trim(),pin,avatar};
      ...
      if(u.pin!==pin){setErr("Incorrect PIN.");return;}
      onLogin({username:u.username,avatar:u.avatar});
    }
  };
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#667eea,#764ba2)",display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      <div style={{background:"#fff",borderRadius:22,padding:"2rem",width:"100%",maxWidth:360,boxShadow:"0 8px 40px rgba(0,0,0,.18)"}}>
        ...
        <div style={{display:"flex",background:"#f5f5f5",borderRadius:12,padding:4,marginBottom:"1.25rem"}}>
          {["login","signup"].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setErr("");}} style={{flex:1,padding:"8px",borderRadius:9,border:"none",cursor:"pointer",fontWeight:600,fontSize:14,background:mode===m?"#fff":"transparent",color:mode===m?"#534AB7":"#888",transition:"all .2s"}}>
        ...
        <button onClick={submit} style={{...S.btn,width:"100%",padding:"11px",borderRadius:10,background:"linear-gradient(135deg,#667eea,#764ba2)",fontSize:15}}>
          {mode==="login"?"Log In 🚀":"Create Account 🎉"}
        </button>
        <div style={{textAlign:"center",marginTop:10,fontSize:12,color:"#aaa"}}>Data saved on this device</div>
      </div>
    </div>
  );
}
...
// ── MAP ───────────────────────────────────────────────────────
function MapView({restaurants,saved,reactions,notes}) {
  ...
  useEffect(()=>{
    ...
    if(!mapInst.current){
      mapInst.current=window.L.map(mapRef.current).setView([-31.952,115.861],13);
      window.L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap",maxZoom:19}).addTo(mapInst.current);
    }
  ...
  return (
    <div style={{height:"calc(100vh - 117px)",display:"flex",flexDirection:"column"}}>
      {!ready&&<div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",color:"#aaa"}}>Loading map...</div>}
          ...
          <div style={{position:"absolute",bottom:12,left:12,right:12,zIndex:400,background:"#fff",borderRadius:12,padding:"12px 14px",boxShadow:"0 2px 12px rgba(0,0,0,.15)"}}>
            <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:12,color:"#666",marginTop:2}}>{selected.vibe} · 📍 {selected.suburb}</div>
                <div style={{fontSize:12,color:"#3B6D11",fontWeight:600,marginTop:2}}>{PRICE_SYM[selected.price]} {PRICE_LABELS[selected.price]}</div>
              </div>
              <button onClick={()=>setSelected(null)} style={{background:"none",border:"none",fontSize:18,color:"#aaa",cursor:"pointer",padding:"0 4px"}}>✕</button>
            </div>
          </div>
        )}
      </div>
      {Object.values(saved).filter(Boolean).length===0&&(
        <div style={{position:"absolute",top:"40%",left:0,right:0,textAlign:"center",color:"#888",pointerEvents:"none"}}>
          <div style={{fontSize:32,marginBottom:8}}>🔖</div>
          <div style={{fontWeight:600}}>Save restaurants to see them on the map</div>
        </div>
      )}
    </div>
  );
...
function DicePage({restaurants,saved,reactions}) {
  ...
  const rxn=restaurant?reactions[restaurant.id]:null;
  const rxnE=rxn?REACTIONS.find(([k])=>k===rxn)?.[1]:"";
  ...
  return (
    <div style={{padding:"1.5rem 1rem",maxWidth:480,margin:"0 auto",textAlign:"center"}}>
      ...
      {cuisine&&(
        <div style={{background:bg(cuisine),border:`2px solid ${tc(cuisine)}`,borderRadius:14,padding:"1rem",marginBottom:10,animation:"fadeUp .4s ease"}}>
      ...
      {restaurant&&(
        <div style={{background:"#fff",border:"2px solid #534AB7",borderRadius:14,padding:"1rem",marginBottom:10,animation:"fadeUp .4s ease",boxShadow:"0 4px 16px rgba(83,74,183,.15)"}}>
      ...
      <div style={{marginTop:8}}>
        {phase==="idle"&&<button onClick={rollCuisine} disabled={rolling} style={{...S.btn,padding:"12px 32px",borderRadius:12,background:"linear-gradient(135deg,#667eea,#764ba2)",fontSize:15,boxShadow:"0 4px 14px rgba(102,126,234,.4)"}}>🎲 Roll for Cuisine!</button>}
...
// ── CUISINE MANAGER ───────────────────────────────────────────
function CuisinePage({customCuisines,setCustomCuisines,setRestaurants,filterCuisine,setFilterCuisine}) {
  ...
  const add = () => {
    ...
    setCustomCuisines(p=>[...p,v]);setNewC("");
  };
  const del = c => { setCustomCuisines(p=>p.filter(x=>x!==c)); if(filterCuisine===c)setFilterCuisine("All"); };
  const save = old => {
...
export default function App() {
  ...
  const [restaurants,setRestaurants] = useState(INIT_RESTAURANTS);
  ...
  const setReaction = (id,r) => setReactions(p=>({...p,[id]:p[id]===r?null:r}));
  ...
  const addAllAI = () => { aiResults.forEach(r=>{const id=Date.now()+Math.random();setRestaurants(p=>[...p,{...r,id}]);setSaved(s=>({...s,[id]:true}));}); setAiResults([]); };
  ...
  const submitForm = () => {
    ...
    const id=Date.now();
    setRestaurants(p=>[...p,{id,name:form.name.trim(),cuisine:form.cuisine,suburb:form.suburb,price:form.price,meal:form.meal,vibe:form.vibe.trim()||"Added by you",rating:parseFloat(form.rating)||null}]);
    setSaved(s=>({...s,[id]:true}));setForm(BLANK_FORM);setFormErr("");setShowAdd(false);
  };
    ...
    return (
      <div style={{background:"#fff",border:saved[r.id]?"1.5px solid #AFA9EC":"1px solid #e5e5e5",borderRadius:14,overflow:"hidden"}}>
        <div style={{position:"relative"}}>
          ...
          {gRating&&<div style={{position:"absolute",top:8,right:8,background:"rgba(0,0,0,.6)",color:"#fff",borderRadius:20,padding:"3px 8px",fontSize:12,fontWeight:600}}>⭐ {gRating} <span style={{fontSize:10,opacity:.8}}>Google</span></div>}
        </div>
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",background:"#f7f7f9",minHeight:"100vh",color:"#222"}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.4)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
          <div style={{background:"#fff",borderRadius:16,padding:"1.5rem",width:"100%",maxWidth:400,boxShadow:"0 4px 24px rgba(0,0,0,.15)"}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:12}}>
              <button style={{...S.btn,flex:1}} onClick={()=>{setNotes(n=>({...n,[notesTarget.id]:notesDraft.trim()}));setNotesTarget(null);}}>Save note</button>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #eee",padding:"12px 1rem",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:150}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button onClick={()=>setUser(null)} style={{background:"none",border:"1px solid #ddd",borderRadius:8,padding:"4px 10px",fontSize:12,color:"#888",cursor:"pointer"}}>Log out</button>
        </div>
      </div>

      {/* Content */}
      <div style={{paddingBottom:70}}>
        {tab==="map"&&<MapView restaurants={restaurants} saved={saved} reactions={reactions} notes={notes}/>}
        {tab==="dice"&&<DicePage restaurants={restaurants} saved={saved} reactions={reactions}/>}
        {tab==="cuisines"&&<CuisinePage customCuisines={customCuisines} setCustomCuisines={setCustomCuisines} setRestaurants={setRestaurants} filterCuisine={fCuisine} setFilterCuisine={setFCuisine}/>}
        {(tab==="browse"||tab==="mylist")&&(
          <div style={{padding:"1rem 1rem 1.5rem",maxWidth:700,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:".85rem",alignItems:"center"}}>
              ...
              {(tab==="mylist"||view==="saved")&&<button style={S.pill(grouped)} onClick={()=>setGrouped(g=>!g)}>🍽️ Group</button>}
            </div>
            ...
            {filtered.length===0?(
              <div style={{textAlign:"center",padding:"3rem 1rem",color:"#aaa"}}>
                ...
                <div style={{fontSize:13,marginTop:6}}>{view==="saved"?"Save some spots first!":"Try adjusting your filters"}</div>
              </div>
            ):grouped&&groupedList?(
